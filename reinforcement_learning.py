...
class Policy_iterator:
    ...
    def pi(self, state):
        ...
        # exploitation
        action_argmax = choice(list(self.moves.keys()))
        for action in self.moves:
            if self.Q.get((state, action), 0) > self.Q.get((state, action_argmax), 0):
        ...
        while self.episodes <= self.max_episodes:
            ...
            if terminal:
                self.Q[(tuple(current_state), action)] = self.Q.get((tuple(current_state), action), 0) + \
                    self.alpha * (reward - self.Q.get((tuple(current_state), action), 0))
            else:  
                self.Q[(tuple(current_state), action)] = self.Q.get((tuple(current_state), action), 0) + \
                    self.alpha * (reward + self.gamma * max([self.Q.get((tuple(next_state), next_action), 0) for next_action in self.moves]) - self.Q.get((tuple(current_state), action), 0))
            
            current_state = next_state
            if terminal:
                self.episodes += 1
                n_games += 1
                n_wins += int(is_win_terminal(current_state, self.number_of_movables))
