...
number_of_movables = len(initial_state)//2
number_of_coordinates = len(initial_state)
...
number_of_candies = len(initial_state) - number_of_coordinates

state_length = number_of_coordinates + number_of_candies
...
def is_terminal(state):
    return np.sum(state[number_of_coordinates:]) == 0 or (state[0] in state[2:number_of_coordinates:2] and state[1] in state[3:number_of_coordinates:2]) 

# For Dynamic Programming we need to enumerate each state
states = np.zeros(shape=(number_of_states, state_length), dtype=np.int8)
...
for pacman_position in possible_positions:
    ...
    # passing the initial state as a numpy array
    new_state = np.array(pacman_position + initial_state[2:], dtype=np.int8)
    # -- Candy eaten check
    for candy_index in range(number_of_coordinates, state_length):
...
print(f"Number of states with everything fixed except Pac-man:\n-\tGeneric states: {state_index}\n-\tOf which terminal: {terminal_states_count} == # Ghosts")

# add all ghosts moves for all pacman moves
for ghost_index in range(1, number_of_movables):
    for s in range(state_index):
        for ghost_position in possible_positions:
            # calculate initial ghost position based on index
            if ghost_position == [initial_state[ghost_index*2], initial_state[ghost_index*2 + 1]]:
                continue    # Such state already exists
            new_state = np.copy(states[s])
            new_state[ghost_index*2] = ghost_position[0]
            new_state[ghost_index*2 + 1] = ghost_position[1]
            if is_terminal(new_state):
                terminal_states_count += 1
            states[state_index] = new_state
            state_index += 1
...
for candy_index in range(number_of_coordinates, len(initial_state)):
    for s in range(state_index):
        if states[s][candy_index] == 1:
            new_state = np.copy(states[s])
            new_state[candy_index] = 0
            if np.sum(new_state[number_of_coordinates:]) == 0:
                if [new_state[0], new_state[1]] in [candies_positions[i] for i in range(number_of_coordinates, len(initial_state))]:
                    terminal_states_count += 1
                    states[state_index] = new_state
                    state_index += 1
            else:
...
# Build unique states (just testing if enumeration logic works)
'''
unique_states = []
for s in states:
...
moves = {
    ...
    4: np.array([0, 0], dtype=np.int8)   # stay
}
...
def pacman_move(state, action):
    eaten=False
    # current position
    pacman_position = state[0:2]
    # next position
    new_position = pacman_position + action
    ...
    if map[new_position[1]][new_position[0]] == 1:
        ...
        new_state[1] = new_position[1]
        # candy eaten check
        for candy_index in range(number_of_coordinates, state.shape[0]):
            if [new_position[0], new_position[1]] == candies_positions[candy_index]:
                new_state[candy_index] = 0
                eaten=True
...
def ghost_move(state, ghost_index):
    ghost_position = state[ghost_index*2:ghost_index*2 + 2]
    actions = [moves[a] for a in moves]
    possible_actions = []
    for a in actions:
        new_position = ghost_position + a   # column vector
...
# cost function
def g(state, eaten):
    # if pacman is eaten by a ghost the cost is -1000
    if state[0] in state[2:number_of_coordinates:2] and state[1] in state[3:number_of_coordinates:2]:
        return 1000
    # if pacman eats a candy the cost is -10
    if eaten:
        if np.sum(state[number_of_coordinates:]) == 0:
            return -1000
        return -10
    # if pacman moves the cost is -1
    return 1
...
def is_win_terminal(state):
    return sum(state[number_of_movables:]) == 0
...
# Value function
previous_value_function = {tuple(s.tolist()) : 1 for s in states}
value_function = {tuple(s.tolist()) : 0 for s in states}

# Policy
policy = np.zeros(shape=(len(states)), dtype=np.int8)
...
while difference_norm >= delta:
    ...
    for s in range(states.shape[0]):
        ...
        for pacman_action in moves:
            ...
            if type(next_state) == bool or (is_terminal(states[s]) and pacman_action != 4):
            ...
            for actions in permuted_actions:
                ...
                for i in range(len(actions)):
                    ghosts_state.append(actions[i] + next_state[(1+i)*2:(i+1)*2 + 2])
                next_state_i = np.copy(next_state)
                for i in range(len(ghosts_state)):
                    # ! seems to work to me !
                    next_state_i[(1+i)*2:(i+1)*2 + 2] = ghosts_state[i]
            ...
            next_state_values = [previous_value_function[tuple(s.tolist())] for s in next_states]
        ...
        value_function[tuple(states[s].tolist())] = min(expected_values)
...
#    print(f"{states[i]} --> {moves[policy[0][i]]}")
...
def store(value_function, policy, map_name):
    ...
    with open(map_name + '_policy.txt', 'w') as f:
        for i in range(len(states)):
            f.write(f"{states[i]} --> {moves[policy[0][i]]}\n")
...
def load(map_name):
    ...
    value_function = {}
    states_number = 0
    with open(map_name + '_value_function.txt', 'r') as f:
        for line in f:
            key, value = line.split(" : ")
            value_function[key] = float(value)
            states_number += 1
    #loading the policy
    policy = np.zeros(shape=(1,states_number), dtype=int)
    with open(map_name + '_policy.txt', 'r') as f:
        for i in range(states_number):
            ...
            policy[0][i] = moves_to_policy[action]
...
def play():
    ...
    while is_terminal(state) == False:
        ...
        pacman_action = policy[0][states.index(state)]
...
#store(value_function, policy, 'ez_map')
